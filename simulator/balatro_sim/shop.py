...
def _create_card(
    ...
    center_key = rng.random_element(pool_key, pool)

    # Resample if UNAVAILABLE
    it = 1
    while center_key == "UNAVAILABLE":
        it += 1
        center_key = rng.random_element(f"{pool_key}_resample{it}", pool)
...
def create_card_for_shop(
    ...
    # Lua: pseudorandom(pseudoseed('cdt'..ante)) * total_rate
    polled_rate = rng.pseudorandom(f"cdt{ante}") * total_rate
...
def get_pack(
    ...
    """Port of get_pack(). First shop always returns a Buffoon Pack.

    Uses weighted selection over PACK_DEFS, matching Lua's iteration order.
    """
    # First shop: forced Buffoon Pack
    # Lua uses math.random(1,2) which is NOT seeded by pseudorandom.
    # For determinism we always pick _1. In practice this doesn't matter
    # since both variants are identical (same weight/cost/config).
    ...
    poll = rng.pseudorandom(f"{key}{ante}") * cume
    it = 0.0
    for p in PACK_DEFS:
...
def get_next_voucher_key(
    ...
    """Port of get_next_voucher_key() from common_events.lua:1901."""
    pool, pool_key = _build_pool("Voucher", None, config, ante)
    if from_tag:
        pool_key = "Voucher_fromtag"

    center = rng.random_element(pool_key, pool)
    it = 1
    while center == "UNAVAILABLE":
        it += 1
        center = rng.random_element(f"{pool_key}_resample{it}", pool)
